...
class FeaturesManager:
    ...
    def get_aqe_features_checkpoint(self, model, df, features):
        checkpoint = model.get_checkpoint()
        checkpoint['data'] = df
    ...
    def save_aqe_features_checkpoint(self, model, df, features):
        features_file_path = self.get_aqe_features_file_path(model.get_name())
        aqe_features_checkpoint = self.get_aqe_features_checkpoint(model, df, features)        
...
def get_pending_features_model(features_manager, model_manager):
    ...
    for model_name in models_list:
        if not features_manager.is_raw_feature_saved(model_name) or not features_manager.is_aqe_feature_saved(model_name):
