...
from pretrained_models import PretainedModels
...
def main(config):
    ...
    pretrained_list_models = pretrained_models.get_pretrained_models_names()
    ...
    if len(pending_models_extract) > 0 :
        ...
        for model_name in pending_models_extract:
            ...
            if not os.path.isfile(features_file):
                ...
                #normalize features            
                features = pretrained_models.postprocessing_features(features,config["PCAdimension"]) 
            ...
            if not os.path.isfile(features_file) or recalc_aqe:
                ...
                features_aqe = pretrained_models.Average_Query_Expansion(features,q_indx,config["top_k_image"])
    ...
    if  config["retrieval_test"] == "True":
        ...
        for index in testid:
            ...
            for model_name in pretrained_list_models:
                ...
                # Cosine Similarity
                ranking = pretrained_models.Cosine_Similarity(features,imgidx,config["top_k_image"])
                ...
                ranking = pretrained_models.Cosine_Similarity(features_aqe,imgidx,config["top_k_image"])
